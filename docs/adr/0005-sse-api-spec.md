# SSE 接口规范：统一格式 + 流式 + 双端点 + 取消机制

决定 Axum 对前端输出统一的 SSE 格式，隐藏背后的 DeepSeek tool calling 过程。`POST /api/chat` 处理对话，`POST /api/cancel` 取消进行中分析。
...
- **端点**：`POST /api/chat`（统一对话入口）+ `POST /api/cancel`（取消进行中分析）
...
- SSE 格式固定，前端可稳定解析
- future 如需暴露 tool calling 中间状态（如"正在查询工程量数据..."），可在 `content` 事件中携带 `status` 子类型扩展，无需改协议
- **取消流程**：收到 `/api/cancel` → `CancellationToken::cancel()` → 流式循环和 tool 执行间隙检测 → 安全终止 goroutine
